...
class DCGAN(object):
    ...
    def __init__(self, epochs, batch_size, device, save_path, G_type="vanilla_gan", D_type="vanilla_gan",
                 ...
                 l1_coef = 50, l2_coef = 100, train_bird=False):
        ...
        if self.G_type == "vanilla_gan":
            ...
            if train_bird:
                self.G.load_state_dict(torch.load("ckpt/G_final.pth"))
        ...
        if self.D_type == "vanilla_gan":
            ...
            if train_bird:
                self.flower_D = cgan.Discriminator(embed_size=embed_size).to(device)
                self.flower_D.load_state_dict(torch.load("ckpt/D_final.pth"))
        ...
        if train_bird:
            self.d_flow_opt = torch.optim.Adam(self.flower_D.parameters(), lr=0.0, betas=(d_beta1, d_beta2))
    ...
    def train(self, flower_data_loader, flower_dataset, bird_dataset, bird_data_loader=None):
        disc_loss = []
        genr_loss = []
        train_loader = bird_data_loader if self.train_bird else flower_data_loader
        for epoch in range(self.epochs):
            for i, batch_data in enumerate(tqdm(train_loader)):
                # Load data
                right_images = batch_data['right_images'].to(self.device)
                size = right_images.size(0)
                ...
                if self.G_type == "vanilla_gan":
                    ...
                    fake_images = self.G(z, right_embed)
                    if self.train_bird:
                        batch_data = next(iter(DataLoader(flower_dataset, batch_size=size, shuffle=True)))
                        flower_right_embed = batch_data['right_embed'][:size].to(self.device)
                        flower_fake = self.G(z, flower_right_embed)


                # Train Generator
                self.G.zero_grad()
                self.flower_D.zero_grad()
                if self.D_type == "vanilla_gan":
                    fake_logits = self.D(fake_images)
                elif self.D_type in ["cgan", "wgan"]:
                    fake_logits, activation_fake = self.D(fake_images, right_embed)
                    if self.train_bird:
                        fake_flower_logits, _ = self.flower_D(flower_fake, flower_right_embed)
                g_loss = self.loss(fake_logits.squeeze(), real_labels)
                if self.train_bird:
                    g_loss += self.loss(fake_flower_logits.squeeze(), real_labels)
            ...
            sample_gen_images = self.generate_img(torch.randn(50, 100, 1, 1).to(self.device), 50, bird_dataset, extra=flower_dataset)
            wandb.log({"generated_images": [wandb.Image(image) for image in sample_gen_images]}, step=(epoch+1)*460)
            if (epoch+1) % 10 == 0:
    ...
    def generate_img(self, z, number_of_images, dataset, text_embedding=None, extra=None):
        if self.G_type == "vanilla_gan":
        ...
        if self.G_type in ["cgan", "wgan"]:
            if text_embedding is None:
                ...
                if extra is not None:
                    fl_text_embedding = self.get_random_embeddings(number_of_images, extra)
            samples = self.G(z, text_embedding).data.cpu().numpy()[:number_of_images]
            fl_sam = self.G(z, fl_text_embedding).data.cpu().numpy()[:number_of_images]

        generated_images = []
        for sample in samples:
            generated_images.append(sample.reshape(3, 64, 64).transpose(1, 2, 0))
        for sample in fl_sam:
            generated_images.append(sample.reshape(3, 64, 64).transpose(1, 2, 0))
